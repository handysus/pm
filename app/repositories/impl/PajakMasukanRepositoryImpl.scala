...
class PajakMasukanRepositoryImpl @Inject()(dbConfigProvider: DatabaseConfigProvider)(implicit ec: ExecutionContext) extends PajakMasukanRepository {
  ...
  override
  def findAll(): Future[List[PajakMasukan]] = db.run {
    p.to[List].result
  }
  ...
  def create(pajakMasukan: PajakMasukan): Future[Option[PajakMasukan]] = db.run {
    (p
      .map(
        pm => (pm.kodeJenisTransaksi, pm.fgPengganti, pm.noFaktur, pm.masaPajak, pm.tahunPajak,
  ...
  override
  def findById(id: Option[Long]): Future[Option[PajakMasukan]] = db.run {
    filterById(id)
  }

  override
  def update(pm: PajakMasukan): Future[Option[PajakMasukan]] = {
    for{
